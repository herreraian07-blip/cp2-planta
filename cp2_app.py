        ...
        with st.expander(f"⬆ {t('act_archivo')} — Vibraciones"):
            ...
            if vib_file is not None:
                try:
                    ...
                    area2=tag2=nombre2=""
                    nuevos = 0
                    # IMPORTANTE: la deteccion de "ya cargado" es por (tag, tipo, fecha),
                    # no solo por fecha -- antes, si CUALQUIER equipo ya tenia dato ese dia,
                    # se descartaban en silencio las mediciones nuevas de otros equipos en
                    # esa misma fecha, aunque nunca se hubieran cargado.
                    existentes2 = set(get_conn().execute(
                        "SELECT tag,tipo_medicion,fecha FROM vibraciones").fetchall())
                    ...
                    for row in range(5, ws_up.max_row+1):
                        ...
                        for col,fecha in col_fechas2.items():
                            if (tag2,tipo2,fecha) in existentes2: continue
                            ...
                            existentes2.add((tag2,tipo2,fecha))
                            nuevos += 1
            ...
            if elec_file is not None:
                ...
                try:
                    ...
                    if elec_tipo == "Heat Trace":
                            ...
                            if fila_vfd:
                                ...
                                for r in range(inicio, fin+1):
                                    ...
                                    try:
                                        ...
                                        if "UNIQUE" not in str(_e_eh): raise
                                    existentes_eh.add(key)
                        ...
                        hojas_ex_m = set(r[0] for r in get_conn().execute(
                            "SELECT DISTINCT fecha_hoja FROM mediciones_mensual WHERE fecha_hoja IS NOT NULL").fetchall())
                        ...
                        for sh2 in wb_up.sheetnames:
                            ...
                            if fecha_h_m in hojas_ex_m: continue
                            ws2 = wb_up[sh2]
                            tag_actual_m = ""
                            for row2 in range(4, ws2.max_row+1):
                                ...
                                run("""INSERT INTO mediciones_mensual
                                    (fecha,tag,subtag,descripcion,estado_alim,cb_estado,ubicacion,observacion,fecha_hoja)
                                    VALUES (?,?,?,?,?,?,?,?,?)""",
