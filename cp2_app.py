        ...
        with sub_eh:
            try:
                ...
                if len(eh_df) == 0:
                    ...
                    with c3:
                        tags_eh = sorted(res_eh["tag_tablero"].dropna().unique().tolist())
                        tag_eh_sel = st.selectbox("Tablero/Equipo", tags_eh, key="eh_tag_sel") if tags_eh else None

                    st.caption(f"{len(res_eh)} registros históricos")

                    if tag_eh_sel:
                        df_eh_tag = res_eh[res_eh.tag_tablero==tag_eh_sel].sort_values("fecha")
                        df_eh_tag = df_eh_tag[df_eh_tag["corriente_medida"].notna()]
                        if len(df_eh_tag) > 0:
                            desc_eh = df_eh_tag["descripcion"].iloc[0] or tag_eh_sel
                            st.markdown(f"**{tag_eh_sel}** — {desc_eh}")
