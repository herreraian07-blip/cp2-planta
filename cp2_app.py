...
import sqlite3, hashlib, pandas as pd, plotly.express as px, os
...
# ════════════════════════════════════
    ...
    if len(pr_df) > 0:
        ...
        st.markdown("#### 📊 CP2 Procurement Report")
        st.caption("Clasificación automática por etapa, en base al número de PR, comprador asignado y estado. "
                   "Se recalcula sola cada vez que se carga una PR nueva o se actualiza el Excel.")
        ...
        with st.expander("Ver detalle de PRs por etapa"):
            etapa_sel_pr = st.selectbox("Etapa", ORDEN_ETAPAS, key="pr_dash_etapa_sel")
            ...
            for _, pr in sub.iterrows():
                ...
                <div style="background:#111113;border:1px solid #1e1e22;border-radius:6px;
                    ...
                    <span style="color:#555;float:right;font-size:11px">{pr.get('comprador') or '— sin comprador'}</span>
                </div>""", unsafe_allow_html=True)
