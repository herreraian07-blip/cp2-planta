...
PLANTILLA_OT_PATH = str(DB_PATH.parent / "plantilla_ot.xlsx")
...
def generar_excel_listado_ots(destino):
    """Genera el R-MTO-01 completo (hoja 'Listado general') con TODAS las OTs
    de la base de datos, siempre al dia (se genera de cero cada vez, no depende
    de un archivo que se vaya actualizando -- eso se perderia en cada reinicio
    del servidor)."""
    import shutil
    shutil.copy(PLANTILLA_OT_PATH, destino)
    wb = openpyxl.load_workbook(destino)
    ws = wb["Listado general"]

    ots = get_conn().execute("""
        SELECT numero_ot,estado,fecha_creacion,creado_por,asignado_a,tipo_tarea,
               prioridad,titulo,descripcion,fecha_fin,horas_reales,observaciones
        FROM ordenes_trabajo ORDER BY fecha_creacion
    """).fetchall()

    fila = 12
    for i, ot in enumerate(ots, start=1):
        (numero_ot,estado,fecha_creacion,creado_por,asignado_a,tipo_tarea,
         prioridad,titulo,descripcion,fecha_fin,horas_reales,observaciones) = ot
        reparado = "SI" if estado == "completada" else "NO"
        ws.cell(fila,2).value = i
        ws.cell(fila,3).value = (estado or "").upper()
        ws.cell(fila,4).value = _fmt_fecha_ot(fecha_creacion)
        ws.cell(fila,5).value = creado_por or ""
        ws.cell(fila,6).value = asignado_a or ""
        ws.cell(fila,7).value = numero_ot or ""
        ws.cell(fila,8).value = tipo_tarea or ""
        ws.cell(fila,9).value = (prioridad or "").upper()
        ws.cell(fila,10).value = f"{titulo or ''} — {descripcion or ''}"
        ws.cell(fila,11).value = _fmt_fecha_ot(fecha_fin)
        ws.cell(fila,12).value = f"{horas_reales} h" if horas_reales else ""
        ws.cell(fila,13).value = reparado
        ws.cell(fila,14).value = observaciones or ""
        fila += 1

    wb.save(destino)
    return destino

def generar_excel_ot_individual(destino, ot_row, equipo_desc="", area_desc=""):
    """Genera el R-MTO-01 (hoja 'Nombre del Registro') para UNA orden de trabajo
    puntual, en el formato oficial de impresion."""
    import shutil
    shutil.copy(PLANTILLA_OT_PATH, destino)
    wb = openpyxl.load_workbook(destino)
    ws = wb["Nombre del Registro"]

    ws["P7"] = f"N° {ot_row['numero_ot']}"
    ws["B8"] = f"FECHA DE EMISION: {_fmt_fecha_ot(ot_row['fecha_creacion'])}"
    ws["B9"] = f"REQUERIDO POR: {ot_row['creado_por'] or ''}"
    ws["J9"] = f"APROBADO POR: {ot_row['supervisor_nombre'] or ''}"
    ws["B10"] = f"PRIORIDAD: {ot_row['prioridad'] or ''}"
    ws["B11"] = f"EQUIPO/AREA: {ot_row['tag_equipo'] or ''} — {equipo_desc} ({area_desc})"
    ws["B12"] = f"DESCRIPCION DEL PROBLEMA: {ot_row['descripcion'] or ''}"
    ws["B13"] = f"FECHA DE RECEPCION: {_fmt_fecha_ot(ot_row['fecha_creacion'])}"
    ws["B14"] = f"SUPERVISOR: {ot_row['supervisor_nombre'] or ''}"
    ws["J14"] = f"SECCION: {ot_row['seccion'] or ''}"
    ws["B15"] = f"TAREAS A REALIZAR: {ot_row['titulo'] or ''}"
    ws["B16"] = f"MATERIALES NECESARIOS: {ot_row['materiales_necesarios'] or ''}"
    ws["B17"] = f"PERSONAL ASIGNADO: {ot_row['asignado_a'] or ''}"
    ws["B18"] = f"FECHA Y HORA DE INICIO: {_fmt_fecha_ot(ot_row['fecha_inicio'])}"
    ws["B19"] = f"FECHA Y HORA DE FINALIZACION ESPERADA: {_fmt_fecha_ot(ot_row['fecha_vencimiento'])}"
    ws["B20"] = f"SERVICIO VERIFICADO: {ot_row['servicio_verificado_por'] or ''}"
    ws["J20"] = f"RESPONSABLE: {ot_row['asignado_a'] or ''}"
    ws["B21"] = f"SUMARIO DEL SERVICIO EJECUTADO: {ot_row['accion_tomada'] or ''}"
    ws["B22"] = f"FECHA Y HORA DE TERMINACION DEL SERVICIO: {_fmt_fecha_ot(ot_row['fecha_fin'])}"
    ws["B23"] = f"COMENTARIOS: {ot_row['observaciones'] or ''}"

    wb.save(destino)
    return destino


def _chequear_archivo_protegido(datos_bytes, nombre_archivo=""):
    """Si el archivo es en realidad un .xlsx/.pptx protegido con DRM (formato OLE/
    Compound File en vez de ZIP), avisa con un mensaje claro en vez del error
    tecnico generico de openpyxl ('File is not a zip file')."""
    if datos_bytes[:4] == b'\xd0\xcf\x11\xe0':
        raise ValueError(
            f"El archivo{' '+nombre_archivo if nombre_archivo else ''} está protegido con DRM "
            "(política corporativa de Microsoft 365) y no se puede leer así. Abrilo en Excel, andá a "
            "Archivo → Información → Proteger libro → Restringir acceso → Acceso sin restricciones, "
            "guardá una copia, y subí esa copia.")
...
def _qdf_ejecutar(sql, params=()):
    cur = get_conn().execute(sql, params)
    cols = [d[0] for d in cur.description] if cur.description else []
    rows = cur.fetchall()
    return [tuple(r) for r in rows], cols

# Cache corto (15s): evita repetir la misma consulta por HTTP a Turso en cada
# rerun/click, sin arriesgar mostrar datos viejos por mucho tiempo. Se limpia
# al instante cada vez que se escribe algo (ver run()), asi que un alta/edicion
# se ve reflejada enseguida igual.
_qdf_cacheada = st.cache_data(ttl=15, show_spinner=False)(_qdf_ejecutar)

def qdf(sql, params=()):
    rows, cols = _qdf_cacheada(sql, params)
...
def migrate():
    try:
        ...
        # migrate() es idempotente (solo agrega columnas/tablas si faltan). Si la
        # conexion remota (Turso) tiene un problema puntual con PRAGMA u otro
        # comando de esquema, no debe tumbar toda la app -- las columnas/tablas
        # ya deberian existir de ejecuciones anteriores.
        pass
...
T = {
    ...
    "pr_filtro_solicitante": {"es":"Solicitante","en":"Requester","ko":"요청자"},
    "pr_filtro_estado": {"es":"Estado","en":"Status","ko":"상태"},
    "pr_filtro_todos":  {"es":"Todos","en":"All","ko":"전체"},
    "pr_ordenar_por":   {"es":"Ordenar por","en":"Sort by","ko":"정렬 기준"},
    "pr_orden_default": {"es":"Etapa (por defecto)","en":"Stage (default)","ko":"단계 (기본값)"},
    "pr_orden_sol_desc":{"es":"Fecha de solicitud (más reciente primero)","en":"Request date (newest first)","ko":"요청일 (최신순)"},
    "pr_orden_sol_asc": {"es":"Fecha de solicitud (más antigua primero)","en":"Request date (oldest first)","ko":"요청일 (오래된순)"},
    "pr_orden_apr_desc":{"es":"Fecha de aprobación (más reciente primero)","en":"Approval date (newest first)","ko":"승인일 (최신순)"},
    "pr_orden_apr_asc": {"es":"Fecha de aprobación (más antigua primero)","en":"Approval date (oldest first)","ko":"승인일 (오래된순)"},
    "pr_gen_moneda_lbl":{"es":"Moneda","en":"Currency","ko":"통화"},
    "pr_gen_contrato_lbl":{"es":"Tipo de contrato","en":"Contract type","ko":"계약 유형"},
    "pr_gen_pedido_lbl":{"es":"Tipo de pedido","en":"Order type","ko":"주문 유형"},
    "pr_gen_resp_lbl":  {"es":"Responsable que autoriza","en":"Approving manager","ko":"승인 담당자"},
    "pr_gen_falta_tit": {"es":"Falta el título.","en":"Title is missing.","ko":"제목이 없습니다."},
    "pr_gen_sin_resp":  {"es":"No hay responsables coreanos cargados para autorizar.","en":"No Korean managers loaded to authorize.","ko":"승인할 한국인 담당자가 없습니다."},
    "pr_estado_lbl":    {"es":"Estado","en":"Status","ko":"상태"},
    "pr_agregado_ok":   {"es":"Pedido agregado","en":"Request added","ko":"요청이 추가되었습니다"},
    "pr_obligatorios":  {"es":"Solicitante y título son obligatorios","en":"Requester and title are required","ko":"요청자와 제목은 필수입니다"},
    "pr_guardado_ok":   {"es":"Guardado","en":"Saved","ko":"저장되었습니다"},
...
if pagina == "dashboard":
    ...
    try:
        ...
        if len(vib_criticos) > 0:
            ...
            titulo_alerta = {
                "es": f"🚨 {len(vib_criticos)} equipos con vibración CRÍTICA — click para ver el gráfico",
                "en": f"🚨 {len(vib_criticos)} equipment with CRITICAL vibration — click to see the chart",
                "ko": f"🚨 {len(vib_criticos)}개 장비 임계 진동 감지 — 클릭하여 그래프 보기"
            }.get(LANG_D,"")

            st.markdown(f"""
            <div style="background:linear-gradient(135deg,#1a0505,#2a0808);
                        border:1.5px solid #ef4444;border-radius:10px 10px 0 0;
                        padding:14px 18px 6px 18px;margin-bottom:0;
                        box-shadow:0 0 20px rgba(239,68,68,0.15)">
                <div style="font-size:14px;font-weight:700;color:#ef4444;
                            letter-spacing:0.02em">{titulo_alerta}</div>
            </div>
            """, unsafe_allow_html=True)
            _cols_vib_crit = st.columns(min(len(vib_criticos), 6))
            for _i_vc, (_, r) in enumerate(vib_criticos.iterrows()):
                with _cols_vib_crit[_i_vc % len(_cols_vib_crit)]:
                    if st.button(f"🔴 {r['tag']}\n{r['val']} mm/s", key=f"vibcrit_{r['tag']}_{_i_vc}",
                                 use_container_width=True):
                        st.session_state["pagina"] = "mediciones"
                        st.session_state["med_seccion_sel"] = f"⚙ {t('med_mec')}"
                        st.session_state["vib_area_sel"] = "Todas"
                        st.session_state["vib_tag_sel"] = r["tag"]
                        st.session_state["vib_tipo_sel"] = "Todos"
                        st.rerun()
    ...
    # ── PANEL DE ELÉCTRICAS CRÍTICAS (corriente medida > límite/OCR) ─────────
    try:
        elec_criticos = qdf("""
            SELECT area, seccion, tag_equipo, tag_tablero, descripcion,
                   MAX(corriente_medida) val, corriente_limite, fecha
            FROM mediciones_ehouse
            WHERE corriente_limite IS NOT NULL AND corriente_medida IS NOT NULL
                  AND corriente_medida > corriente_limite
            GROUP BY area, seccion, tag_equipo ORDER BY MAX(corriente_medida) DESC
        """)
        if len(elec_criticos) > 0:
            titulo_alerta_e = {
                "es": f"⚡ {len(elec_criticos)} equipos eléctricos con corriente por ENCIMA del límite — click para ver el gráfico",
                "en": f"⚡ {len(elec_criticos)} electrical equipment with current ABOVE the limit — click to see the chart",
                "ko": f"⚡ 한도를 초과한 전기 장비 {len(elec_criticos)}개 — 클릭하여 그래프 보기",
            }.get(st.session_state.get("lang","es"),"")
            st.markdown(f"""
            <div style="background:linear-gradient(135deg,#1a1005,#2a1a08);
                        border:1.5px solid #f59e0b;border-radius:10px 10px 0 0;
                        padding:14px 18px 6px 18px;margin-bottom:0;
                        box-shadow:0 0 20px rgba(245,158,11,0.15)">
                <div style="font-size:14px;font-weight:700;color:#f59e0b;
                            letter-spacing:0.02em">{titulo_alerta_e}</div>
            </div>
            """, unsafe_allow_html=True)
            _cols_elec_crit = st.columns(min(len(elec_criticos), 6))
            for _i_ec, (_, r) in enumerate(elec_criticos.iterrows()):
                _tag_click = r["tag_equipo"] or r["tag_tablero"]
                with _cols_elec_crit[_i_ec % len(_cols_elec_crit)]:
                    if st.button(f"🟡 {_tag_click}\n{r['val']:.1f} A (límite {r['corriente_limite']:.0f} A)",
                                 key=f"eleccrit_{_tag_click}_{_i_ec}", use_container_width=True):
                        st.session_state["pagina"] = "mediciones"
                        st.session_state["med_seccion_sel"] = f"⚡ {t('med_elec')}"
                        st.session_state["med_elec_sub_sel"] = "🏭 E-House (VFD/LOB)"
                        st.session_state["ehouse_area_sel"] = r["area"]
                        st.session_state["eh_area_sel"] = r["area"]
                        st.session_state["eh_seccion_sel"] = r["seccion"]
                        st.session_state["eh_tag_sel"] = _tag_click
                        st.rerun()
            st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
    except:
        pass
...
# ════════════════════════════════════
    ...
    if len(ots_df) > 0:
        if st.button("📥 Descargar registro completo (R-MTO-01)", key="btn_descargar_listado_ot"):
            import tempfile
            tmp = tempfile.mkdtemp()
            ruta = generar_excel_listado_ots(f"{tmp}/R-MTO-01_Listado_OTs.xlsx")
            with open(ruta, "rb") as f:
                st.download_button("⬇ Click para bajar el Excel", f.read(),
                    file_name="R-MTO-01_Listado_de_OTs.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    key="dl_listado_ot")

    c1,c2,c3=st.columns(3)
    with c1: f_est=st.multiselect(t("estado"),["pendiente","en_curso","completada","cancelada"],
                default=["pendiente","en_curso"], format_func=t)
    with c2: f_pri=st.multiselect(t("prioridad"),["critica","alta","normal","baja"], format_func=t)
    with c3: f_tip=st.multiselect(t("tipo"),["correctivo","preventivo","inspeccion","lubricacion","calibracion","mejora"], format_func=t)
    ...
    for _,ot in res_ot.iterrows():
        with st.expander(f"{PRIO_IC.get(ot['prioridad'],'')}  {ot['numero_ot']}  ·  {ot['tag_equipo']}  ·  {ot['titulo']}"):
            ...
            with cbtn2:
                if st.button("📄 Formulario R-MTO-01 (Excel)", key=f"rmto_ot_{ot['id']}"):
                    import tempfile
                    sup_df = qdf("SELECT nombre,apellido FROM usuarios WHERE id=?", (ot.get("supervisor_id"),)) if ot.get("supervisor_id") else None
                    sup_nombre = f"{sup_df.iloc[0]['nombre']} {sup_df.iloc[0]['apellido']}" if sup_df is not None and len(sup_df)>0 else ""
                    eq_df = qdf("SELECT spec_nombre_equipo,area_descripcion FROM equipos WHERE tag=?", (ot["tag_equipo"],))
                    eq_desc = eq_df.iloc[0]["spec_nombre_equipo"] if len(eq_df)>0 else ""
                    area_desc = eq_df.iloc[0]["area_descripcion"] if len(eq_df)>0 else ""
                    ot_dict = dict(ot); ot_dict["supervisor_nombre"] = sup_nombre
                    tmp = tempfile.mkdtemp()
                    ruta = generar_excel_ot_individual(f"{tmp}/R-MTO-01_{ot['numero_ot']}.xlsx", ot_dict, eq_desc, area_desc)
                    with open(ruta, "rb") as f:
                        st.download_button("⬇ Click para bajar el formulario", f.read(),
                            file_name=f"R-MTO-01_{ot['numero_ot']}.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            key=f"dl_rmto_{ot['id']}")
            ...
            if PUEDE_EDITAR:
                ...
                with cu1:
                    _estados_raw = ["pendiente","en_curso","completada","cancelada"]
                    _idx_est = _estados_raw.index(ot["estado"]) if ot["estado"] in _estados_raw else 0
                    n_est=st.selectbox(t("estado"),_estados_raw,index=_idx_est,
                        format_func=t, key=f"est_{ot['id']}")
                with cu2:
...
# ════════════════════════════════════
    ...
    c1,c2=st.columns(2)
    with c1: f_esp=st.multiselect(t("especialidad"),["mecanico","electrico","instrumentacion","otro"], format_func=t)
...
# ════════════════════════════════════
    ...
    if _area_pr in ("Mecanica", "Electricidad"):
        with st.expander("📝 Generar solicitud de compra (Power Point + Excel)"):
            ...
            with cpr3:
                pr_moneda = st.selectbox(t("pr_gen_moneda_lbl"), ["USD","ARS"], key="pr_gen_moneda")
            pr_monto = st.number_input(f"Monto total ({pr_moneda})", min_value=0.0, step=100.0, key="pr_gen_monto")

            c4,c5 = st.columns(2)
            with c4:
                pr_tipo_contrato = st.selectbox(t("pr_gen_contrato_lbl"), ["Non Bidding","Bidding"], key="pr_gen_contrato")
            with c5:
                pr_tipo_pedido = st.selectbox(t("pr_gen_pedido_lbl"),
            ...
            pr_responsable_sel = st.selectbox(t("pr_gen_resp_lbl"), _opciones_resp, key="pr_gen_resp") if _opciones_resp else None
            ...
            if "pr_gen_objetivo" in st.session_state:
                ...
                if st.button("📄 Generar archivos (JPG + Excel)", type="primary", key="pr_gen_btn_final"):
                    if not pr_titulo.strip():
                        st.error(t("pr_gen_falta_tit"))
                    elif not pr_responsable_sel:
                        st.error(t("pr_gen_sin_resp"))
                    else:
                        try:
                            hoy = datetime.now()
    ...
    with st.expander(f"⬆ {t('act_archivo')} — Purchase Requests"):
        ...
        if pr_file is not None:
            ...
            try:
                _bytes_pr = pr_file.read()
                _chequear_archivo_protegido(_bytes_pr, pr_file.name)
                wb_pr = _opx_pr.load_workbook(_io_pr.BytesIO(_bytes_pr), data_only=True)
                ws_pr = wb_pr.active
                def _fmtd(v):
                    if v is None: return ''
    ...
    if len(pr_df) > 0:
        ...
        with fc1:
            opciones_solicitante = [t("pr_filtro_todos")] + sorted(pr_df["solicitante"].dropna().unique().tolist())
            filtro_solicitante = st.selectbox(t("pr_filtro_solicitante"), opciones_solicitante, key="pr_filtro_sol")
        with fc2:
            opciones_estado_pr = [t("pr_filtro_todos")] + sorted(pr_df["estado"].dropna().unique().tolist())
            filtro_estado_pr = st.selectbox(t("pr_filtro_estado"), opciones_estado_pr, key="pr_filtro_est")
        with fc3:
            opciones_orden = [t("pr_orden_default"), t("pr_orden_sol_desc"), t("pr_orden_sol_asc"),
                               t("pr_orden_apr_desc"), t("pr_orden_apr_asc")]
            orden_sel = st.selectbox(t("pr_ordenar_por"), opciones_orden, key="pr_orden_sel")
        ...
        if filtro_solicitante != t("pr_filtro_todos"):
            res_pr = res_pr[res_pr.solicitante == filtro_solicitante]
        if filtro_estado_pr != t("pr_filtro_todos"):
            res_pr = res_pr[res_pr.estado == filtro_estado_pr]

        if orden_sel == t("pr_orden_sol_desc"):
            res_pr = res_pr.sort_values("fecha_solicitud", ascending=False, na_position="last")
        elif orden_sel == t("pr_orden_sol_asc"):
            res_pr = res_pr.sort_values("fecha_solicitud", ascending=True, na_position="last")
        elif orden_sel == t("pr_orden_apr_desc"):
            res_pr = res_pr.sort_values("fecha_aprobacion", ascending=False, na_position="last")
        elif orden_sel == t("pr_orden_apr_asc"):
            res_pr = res_pr.sort_values("fecha_aprobacion", ascending=True, na_position="last")
        # si es "por defecto", res_pr ya viene ordenado por etapa desde pr_df
    ...
    with tab_add:
        with st.form("form_pr_add"):
            ...
            with c3:
                ...
                a_est  = st.selectbox(t("pr_estado_lbl"),
            ...
            if st.form_submit_button("Agregar pedido", type="primary", use_container_width=True):
                if a_tit and a_req:
                    ...
                    st.success(t("pr_agregado_ok")); st.rerun()
                else:
                    st.error(t("pr_obligatorios"))
    ...
    with tab_edit:
        if len(pr_df) > 0:
            ...
            with st.form("form_pr_edit"):
                ...
                with c2:
                    e_est  = st.selectbox(t("pr_estado_lbl"),
                        ["In progress","Pending","In transit","Delivered","Cancelled"],
                ...
                with col_sv:
                    if st.form_submit_button("Guardar cambios", type="primary",
                        ...
                        st.success(t("pr_guardado_ok")); st.rerun()
                with cc2:
...
# ════════════════════════════════════
    ...
    _OPC_MED_SECCION = [f"⚙ {t('med_mec')}", f"⚡ {t('med_elec')}", "🗂 Otras áreas"]
    if "med_seccion_sel" not in st.session_state:
        st.session_state["med_seccion_sel"] = _OPC_MED_SECCION[0]
    med_seccion_sel = st.radio("", _OPC_MED_SECCION, key="med_seccion_sel",
        horizontal=True, label_visibility="collapsed")

    # ═════════════════ TAB MECÁNICAS ═════════════════
    if med_seccion_sel == _OPC_MED_SECCION[0]:
        ...
        with st.expander(f"⬆ {t('act_archivo')} — Vibraciones"):
            ...
            if vib_file is not None:
                try:
                    _bytes_vib = vib_file.read()
                    _chequear_archivo_protegido(_bytes_vib, vib_file.name)
                    wb_up = __import__('openpyxl').load_workbook(_io.BytesIO(_bytes_vib), data_only=True)
                    ws_up = wb_up['Sheet1']
                    from datetime import date as _date
                    TODAY2 = _date.today()
                    col_fechas2 = {}
                    ...
                    for row in range(5, ws_up.max_row+1):
                        ...
                        for col,fecha in col_fechas2.items():
                            ...
                            run("""INSERT INTO vibraciones (tag,equipo_desc,tipo_medicion,valor,unidad,limite,semana,fecha,estado,area)
                                VALUES (?,?,?,?,?,?,?,?,?,?)
                                ON CONFLICT(tag,tipo_medicion,fecha) DO UPDATE SET
                                    equipo_desc=excluded.equipo_desc, valor=excluded.valor, unidad=excluded.unidad,
                                    limite=excluded.limite, estado=excluded.estado, area=excluded.area""",
    ...
    # ═════════════════ TAB ELÉCTRICAS ═════════════════
    elif med_seccion_sel == _OPC_MED_SECCION[1]:
        st.markdown(f"### {t('med_elec')}")

        # Botón actualizar
        with st.expander(f"⬆ {t('act_archivo')} — Eléctrico"):
            st.caption(t("arrastra"))
            elec_tipo = st.radio("Tipo", ["Heat Trace","PLC diario","Inspección semanal","E-House (VFD/LOB)","Inspección mensual"],
            ...
            if elec_file is not None:
                ...
                import re as _re2
                try:
                    _bytes_elec = elec_file.read()
                    _chequear_archivo_protegido(_bytes_elec, elec_file.name)
                    wb_up = _opx.load_workbook(_io2.BytesIO(_bytes_elec), data_only=True)
                    nuevos = 0
                    ...
                    if elec_tipo == "Heat Trace":
                        ...
                        for sh2 in wb_up.sheetnames:
                            ...
                            for col_i,(panel,ubic) in PANELES_UP.items():
                                ...
                                for row2 in range(10,40):
                                    ...
                                    run("""INSERT INTO mediciones_heat_trace
                                        (fecha,panel,ubicacion,voltaje_v,corriente_a,mc_estado,hora_insp,cct,amperaje_a)
                                        VALUES (?,?,?,?,?,?,?,?,?)
                                        ON CONFLICT(fecha,panel,cct) DO UPDATE SET
                                            ubicacion=excluded.ubicacion, voltaje_v=excluded.voltaje_v,
                                            corriente_a=excluded.corriente_a, mc_estado=excluded.mc_estado,
                                            hora_insp=excluded.hora_insp, amperaje_a=excluded.amperaje_a""",
                            ...
                            for fi,ff,plcs2 in BLOQUES_UP:
                                for pid,cn,ce in plcs2:
                                    ...
                                    for row2 in range(fi,ff+1):
                                        ...
                                        run("""INSERT INTO mediciones_plc (fecha,plc_id,modulo,lampara,estado,es_anomalia)
                                            VALUES (?,?,?,?,?,?)
                                            ON CONFLICT(fecha,plc_id,lampara) DO UPDATE SET
                                                modulo=excluded.modulo, estado=excluded.estado, es_anomalia=excluded.es_anomalia""",
                        ...
                        for sh2 in wb_up.sheetnames:
                            ...
                            if fila_vfd:
                                ...
                                for r in range(inicio, fin+1):
                                    ...
                                    lim = _to_float_eh(ws2.cell(r,7).value)
                                    if lim is not None: lim = abs(lim)
                                    estado = ws2.cell(r,8).value
                                    corr = _to_float_eh(ws2.cell(r,9).value)
                                    pct_carga = _to_float_eh(ws2.cell(r,10).value)
                                    frec = _to_float_eh(ws2.cell(r,11).value)
                                    if frec is not None: frec = abs(frec)
                                    run("""INSERT INTO mediciones_ehouse
                                        (area,seccion,fecha,tag_tablero,tag_equipo,descripcion,potencia_kw,corriente_medida,estado,corriente_limite,porcentaje_carga,frecuencia_hz)
                                        VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
                                        ON CONFLICT(area,seccion,fecha,tag_equipo) DO UPDATE SET
                                            tag_equipo=excluded.tag_equipo, descripcion=excluded.descripcion,
                                            potencia_kw=excluded.potencia_kw, corriente_medida=excluded.corriente_medida,
                                            estado=excluded.estado, corriente_limite=excluded.corriente_limite,
                                            porcentaje_carga=excluded.porcentaje_carga, frecuencia_hz=excluded.frecuencia_hz""",
                            ...
                            if fila_lob:
                                ...
                                for r in range(inicio, ws2.max_row+1):
                                    ...
                                    lim = _to_float_eh(ws2.cell(r,6).value)
                                    if lim is not None: lim = abs(lim)
                                    estado = ws2.cell(r,7).value
                                    corr = _to_float_eh(ws2.cell(r,8).value)
                                    ocr = _to_float_eh(ws2.cell(r,10).value)
                                    if ocr is not None: ocr = abs(ocr)
                                    frec = _to_float_eh(ws2.cell(r,11).value)
                                    if frec is not None: frec = abs(frec)
                                    run("""INSERT INTO mediciones_ehouse
                                        (area,seccion,fecha,tag_tablero,tag_equipo,descripcion,potencia_kw,corriente_medida,estado,corriente_limite,porcentaje_carga,frecuencia_hz)
                    ...
                    elif elec_tipo == "Inspección mensual":
                        def _parse_fecha_hoja_m(nombre):
                            s = nombre.strip()
                            for sep in ('.', '-', '/', ','):
                                if sep in s:
                                    partes = [p for p in s.split(sep) if p.strip()]
                                    ...
                                    if len(partes) == 2:
                                        try:
                                            aa, mm = partes
                                            return f"20{aa.strip().zfill(2)}-{mm.strip().zfill(2)}"
                                        except Exception:
                                            continue
                            return None

                        for sh2 in wb_up.sheetnames:
                            if sh2.strip().lower() in ('libre',): continue
                            fecha_h_m = _parse_fecha_hoja_m(sh2)
                            if fecha_h_m is None: continue
                            ws2 = wb_up[sh2]
                            tag_actual_m = ""
                            for row2 in range(4, ws2.max_row+1):
                                v_no = ws2.cell(row2,1).value
                                if v_no is None: continue
                                try: numero_item_m = int(float(v_no))
                                except: continue
                                v_tag = ws2.cell(row2,2).value
                                v_subtag = ws2.cell(row2,3).value
                                if v_tag and str(v_tag).strip(): tag_actual_m = str(v_tag).strip()
                                if not tag_actual_m: continue
                                desc = ws2.cell(row2,4).value
                                fec_e = ws2.cell(row2,5).value
                                fecha_r_m = fec_e.strftime('%Y-%m-%d') if hasattr(fec_e,'strftime') else fecha_h_m
                                estado_alim = ws2.cell(row2,6).value
                                cb_estado = ws2.cell(row2,7).value
                                ubicacion = ws2.cell(row2,8).value
                                obs = ws2.cell(row2,9).value
                                run("""INSERT INTO mediciones_mensual
                                    (fecha,tag,subtag,descripcion,estado_alim,cb_estado,ubicacion,observacion,fecha_hoja,numero_item)
                                    VALUES (?,?,?,?,?,?,?,?,?,?)
                                    ON CONFLICT(fecha_hoja,numero_item) DO UPDATE SET
                                        fecha=excluded.fecha, tag=excluded.tag, subtag=excluded.subtag,
                                        descripcion=excluded.descripcion, estado_alim=excluded.estado_alim,
                                        cb_estado=excluded.cb_estado, ubicacion=excluded.ubicacion,
                                        observacion=excluded.observacion""",
                                    (fecha_r_m, tag_actual_m,
                                     str(v_subtag).strip() if v_subtag else None,
                                     str(desc).strip() if desc else None,
                                     str(estado_alim).strip() if estado_alim else None,
                                     str(cb_estado).strip() if cb_estado else None,
                                     str(ubicacion).strip() if ubicacion else None,
                                     str(obs).strip() if obs else None,
                                     fecha_h_m, numero_item_m))
                                nuevos += 1
                    else:
                        ...
                        for sh2 in wb_up.sheetnames:
                            ...
                            for row2 in range(4,ws2.max_row+1):
                                ...
                                run("""INSERT INTO mediciones_semanal
                                    (fecha,tag,descripcion,estado_alim,valor,valor_num,observacion,tipo,fecha_hoja)
                                    VALUES (?,?,?,?,?,?,?,?,?)
                                    ON CONFLICT(fecha_hoja,tag,descripcion) DO UPDATE SET
                                        fecha=excluded.fecha, estado_alim=excluded.estado_alim,
                                        valor=excluded.valor, valor_num=excluded.valor_num,
                                        observacion=excluded.observacion""",
        ...
        # Sub-secciones (antes eran st.tabs, cambiado a radio para poder
        # navegar directo desde el Dashboard con un click)
        _OPC_MED_ELEC_SUB = ["🔌 Heat Trace", f"🖥 {t('plc_diario')}", f"📋 {t('insp_semanal')}", "🏭 E-House (VFD/LOB)"]
        if "med_elec_sub_sel" not in st.session_state:
            st.session_state["med_elec_sub_sel"] = _OPC_MED_ELEC_SUB[0]
        med_elec_sub_sel = st.radio("", _OPC_MED_ELEC_SUB, key="med_elec_sub_sel",
            horizontal=True, label_visibility="collapsed")

        # ─── HEAT TRACE ───────────────────────────────────────────────────────
        if med_elec_sub_sel == _OPC_MED_ELEC_SUB[0]:
            try:
                ...
                if len(ht_df)==0:
                    ...
                    # Voltaje del panel a lo largo del tiempo (el voltaje se mide 1 vez por panel/dia, no por CCT)
                    df_volt = df_panel[["fecha","voltaje_v"]].dropna(subset=["voltaje_v"]).drop_duplicates(subset=["fecha"]).copy()
                    if len(df_volt) > 0:
                        df_volt["fecha"] = pd.to_datetime(df_volt["fecha"])
                        st.markdown(f"#### Voltaje del panel — {panel_sel}")
                        fig_v = px.line(df_volt.sort_values("fecha"), x="fecha", y="voltaje_v",
                            markers=True, height=220, color_discrete_sequence=["#f59e0b"],
                            labels={"fecha":"Fecha","voltaje_v":"Voltaje (V)"})
                        fig_v.update_layout(margin=dict(l=0,r=0,t=10,b=0),
                            plot_bgcolor="rgba(0,0,0,0)",paper_bgcolor="rgba(0,0,0,0)",
                            font=dict(family="Inter",size=11,color="#888"),
                            xaxis=dict(color="#444",gridcolor="#1e1e22"),
                            yaxis=dict(color="#444",gridcolor="#1e1e22",title="V"))
        ...
        # ─── PLC DIARIO ───────────────────────────────────────────────────────
        elif med_elec_sub_sel == _OPC_MED_ELEC_SUB[1]:
            try:
        ...
        # ─── INSPECCIÓN SEMANAL ───────────────────────────────────────────────
        elif med_elec_sub_sel == _OPC_MED_ELEC_SUB[2]:
            try:
        ...
        # ─── E-HOUSE (VFD / LOB) ──────────────────────────────────────────────
        elif med_elec_sub_sel == _OPC_MED_ELEC_SUB[3]:
            try:
                ...
                if len(eh_df) == 0:
                    ...
                    if tag_eh_sel:
                        ...
                        if seccion_eh_sel == "LOB":
                            # MCC/LOB: solo interesa la corriente cuando el equipo esta en ON/START
                            # (es cuando se ve la corriente de arranque real, no 0 en reposo)
                            df_eh_tag = df_eh_tag[df_eh_tag["estado"].fillna("").str.contains("START", case=False)]
                        ...
                        if len(df_eh_tag) > 0:
                            ...
                            if seccion_eh_sel == "LOB":
                                st.caption("Mostrando solo mediciones tomadas en estado ON / START (corriente de arranque)")

                            from plotly.subplots import make_subplots
                            import plotly.graph_objects as go
                            fig_eh = make_subplots(specs=[[{"secondary_y": True}]])
                            fig_eh.add_trace(go.Scatter(x=df_eh_tag["fecha"], y=df_eh_tag["corriente_medida"],
                                name="Corriente [A]", mode="lines+markers", line=dict(color="#0ea5e9"),
                                marker=dict(size=6)), secondary_y=False)

                            _lim_col = "corriente_limite"
                            _lim_label = "Sobrecorriente OCR [A]" if seccion_eh_sel == "LOB" else "Corriente nominal interruptor [A]"
                            if df_eh_tag[_lim_col].notna().any():
                                _lim_val = df_eh_tag[_lim_col].dropna().iloc[0]
                                fig_eh.add_trace(go.Scatter(x=df_eh_tag["fecha"], y=[_lim_val]*len(df_eh_tag),
                                    name=_lim_label, mode="lines", line=dict(color="#ef4444", dash="dash")),
                                    secondary_y=False)

                            if df_eh_tag["frecuencia_hz"].notna().any():
                                fig_eh.add_trace(go.Scatter(x=df_eh_tag["fecha"], y=df_eh_tag["frecuencia_hz"],
                                    name="Frecuencia [Hz]", mode="lines+markers", line=dict(color="#a78bfa"),
                                    marker=dict(size=5)), secondary_y=True)

                            fig_eh.update_layout(height=320, margin=dict(l=0,r=0,t=10,b=0),
                                plot_bgcolor="rgba(0,0,0,0)",paper_bgcolor="rgba(0,0,0,0)",
                                font=dict(family="Inter",size=11,color="#888"),
                                legend=dict(orientation="h", y=-0.15))
                            fig_eh.update_xaxes(color="#444",gridcolor="#1e1e22")
                            fig_eh.update_yaxes(title_text="Corriente [A]", color="#444",gridcolor="#1e1e22", secondary_y=False)
                            fig_eh.update_yaxes(title_text="Frecuencia [Hz]", color="#444",gridcolor="#1e1e22", secondary_y=True)
                            ...
                            st.info("Sin mediciones numéricas de corriente para este equipo en el período/estado seleccionado")

                    st.markdown("---")
                    st.markdown("#### ⚡ Consumo total y potencia por E-Room")
                    st.caption("Cálculo trifásico aproximado: S = √3 × V × I (potencia aparente). "
                               "No se cuenta con el factor de potencia (cos φ) medido, por lo que la Potencia "
                               "Real (P) se estima igual a la aparente — es un valor orientativo, no exacto.")
                    _ultima_fecha_area = res_eh["fecha"].max() if len(res_eh)>0 else None
                    if _ultima_fecha_area:
                        _df_pot = res_eh[(res_eh["fecha"]==_ultima_fecha_area) & (res_eh["corriente_medida"].notna())].copy()
                        _V_ASUMIDO = 380.0  # voltaje tipico de tablero trifasico en planta; ajustar si se conoce el real
                        _df_pot["S_kVA"] = (3**0.5) * _V_ASUMIDO * _df_pot["corriente_medida"] / 1000

                        resumen_shelter = _df_pot.groupby(["area","seccion"]).agg(
                            corriente_total_A=("corriente_medida","sum"),
                            S_total_kVA=("S_kVA","sum"),
                            equipos_activos=("corriente_medida","count")).reset_index()
                        st.dataframe(resumen_shelter.rename(columns={
                            "area":"E-Room","seccion":"Sección","corriente_total_A":"Corriente total [A]",
                            "S_total_kVA":"Potencia aparente S total [kVA]","equipos_activos":"Equipos con lectura"}),
                            use_container_width=True, hide_index=True)

                        c1,c2,c3 = st.columns(3)
                        _df_vfd_tot = _df_pot[_df_pot["seccion"]=="VFD"]
                        with c1:
                            st.metric("Corriente total — solo VFD activos", f"{_df_vfd_tot['corriente_medida'].sum():.1f} A")
                        with c2:
                            st.metric("Potencia (S) total — solo VFD", f"{_df_vfd_tot['S_kVA'].sum():.1f} kVA")
                        with c3:
                            st.metric(f"Suma TODOS los E-Rooms (VFD+LOB)", f"{_df_pot['S_kVA'].sum():.1f} kVA")
                        st.caption(f"Voltaje asumido para el cálculo: {_V_ASUMIDO:.0f} V trifásico, datos al {_ultima_fecha_area}. "
                                   "Si el voltaje real de algún tablero es distinto, avisá para ajustar el cálculo equipo por equipo.")
    ...
    elif med_seccion_sel == _OPC_MED_SECCION[2]:
