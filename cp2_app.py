...
if pagina == "dashboard":
    ...
    try:
        ...
        if len(elec_criticos) > 0:
            titulo_alerta_e = {
                "es": f"⚡ {len(elec_criticos)} equipos eléctricos con corriente por ENCIMA del límite",
                "en": f"⚡ {len(elec_criticos)} electrical equipment with current ABOVE the limit",
                "ko": f"⚡ 한도를 초과한 전기 장비 {len(elec_criticos)}개",
            }.get(st.session_state.get("lang","es"),"")
            badges_e = "".join([
                f"<span style='background:#1a1005;border:1px solid #92400e;"
                f"border-radius:5px;padding:4px 10px;font-family:monospace;"
                f"font-size:11px;color:#fbbf24;white-space:nowrap;display:inline-block'>"
                f"<b>{r['tag_equipo'] or r['tag_tablero']}</b>&nbsp;&nbsp;{r['val']:.1f} A "
                f"(límite {r['corriente_limite']:.0f} A)</span>"
                for _,r in elec_criticos.iterrows()
            ])
            st.markdown(f"""
            <div style="background:linear-gradient(135deg,#1a1005,#2a1a08);
                ...
                <div style="font-size:14px;font-weight:700;color:#f59e0b;
                            letter-spacing:0.02em;margin-bottom:10px">{titulo_alerta_e}</div>
                <div style="display:flex;flex-wrap:wrap;gap:6px">{badges_e}</div>
            </div>
            """, unsafe_allow_html=True)
    except:
        pass
